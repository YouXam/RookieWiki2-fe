  ...
  <v-app>
    ...
    <v-dialog v-model="login_data.show"  max-width="600px" persistent>
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" text @click="login" :loading="login_data.loading">登录</v-btn>
          <v-btn color="blue darken-1" text @click="login_data.show = false">取消</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="register_data.show"  max-width="600px" persistent>
      <v-card>
        ...
        <v-card-text>
          <v-container>
            <v-form>
              <v-text-field v-model="register_data.username" label="用户名" :rules="nameRules" clearable></v-text-field>
              <v-text-field v-model="register_data.email" label="邮箱" :rules="emailRules" clearable></v-text-field>
              <v-text-field v-model="register_data.password" label="密码" :rules="passwordRules" type="password" clearable></v-text-field>
              <v-text-field v-model="register_data.confirm" label="确认密码" :rules="confirmRules" type="password" clearable></v-text-field>
            </v-form>
          </v-container>
        </v-card-text>
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" text @click="register" :loading="register_data.loading">注册</v-btn>
          <v-btn color="blue darken-1" text @click="register_data.show = false">取消</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
      ...
      <div class='login-box'>
        ...
        <v-menu bottom v-if="islogin" :offset-y="true">
          <template v-slot:activator="{ on, attrs }">
            <v-avatar color="grey darken-1 shrink" size="45" v-bind="attrs" v-on="on" class="hidden-sm-and-down">
    ...
    <v-navigation-drawer v-model="drawer" absolute temporary style="z-index: 99999">
      ...
      <v-list>
        ...
        <v-list-item v-for="link in links" :key="link.to" @click="goto(link.to)">
          <v-list-item-icon>
...
export default {
  data: () => ({
    ...
    nameRules: [
      v => !!v || '用户名不可为空',
      v => /^[^+ /?$#&=]{1,30}$/.test(v) || '用户名必须在30个字符以内, 且不能包含+, /, ?, $, #, &, =和空格'
    ],
    emailRules: [
      v => !!v || '邮箱不可为空',
      v => /^.+@.+\..+$/.test(v) || '邮箱格式不正确'
  ...
  computed: {
    confirmRules: function () {
      return [
        v => !!v || '请再输入一次密码',
        () => this.register_data.password === this.register_data.confirm || '密码不匹配'
      ]
    },
    menu: function () {
      return [
  ...
  methods: {
    ...
    update_info: async function () {
      ...
      this.avatar = `https://sdn.geekzu.org/avatar/${md5(this.email)}?d=404&s=500`
    },
    ...
    register: async function () {
      if (!/^[^+ /?$#&=]{1,30}$/.test(this.register_data.username) ||
        !/^.+@.+\..+$/.test(this.register_data.email) ||
        !/.{6,200}/.test(this.register_data.password) ||
        this.register_data.confirm !== this.register_data.password) return
